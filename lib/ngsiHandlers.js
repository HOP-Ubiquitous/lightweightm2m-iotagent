...
function sendLwm2mOperation(operation, ngsiDevice, lwm2mDevice, attributeName, attributeValue, callback) {
    ...
    // Allow send execute requests without payload (reboot device, reset min/max values, ...)
    if (operation === lwm2mLib.execute) {
...
function ngsiUpdateHandler(id, type, attributes, callback) {
    ...
    function updateDevice(ngsiDevice, lwm2mDevice, innerCallback) {
        if (config.lwm2m.requestSerially && config.lwm2m.requestSerially === true) {
            // Serie request mode
            var queries = [];
                for (var i = 0; i < attributes.length; i++) {
                queries.push(apply(writeAttribute,
                    ngsiDevice,
                    lwm2mDevice,
                    attributes[i]
                ));
            }
            async.series(queries, innerCallback);
        } else {
            // Simultaneous request mode (Default)
            async.map(
                attributes,
...
function ngsiExecuteHandler(id, type, attributes, callback) {
    ...
    function executeCommand(ngsiDevice, lwm2mDevice, innerCallback) {
        if (config.lwm2m.requestSerially && config.lwm2m.requestSerially === true) {
            ...
            var queries = [];
                for (var i = 0; i < attributes.length; i++) {
                queries.push(apply(executeAttribute,
                    ngsiDevice,
                    lwm2mDevice,
                    attributes[i]
                ));
            ...
            async.map(
                attributes,
                async.apply(executeAttribute, ngsiDevice, lwm2mDevice),
                innerCallback);
        }
    }
...
function ngsiQueryHandler(id, type, attributes, callback) {
    ...
    function readAttributes(attributes, ngsiDevice, lwm2mDevice, innerCallback) {
        ...
        if (config.lwm2m.requestSerially && config.lwm2m.requestSerially === true) {
            ...
            var queries = [];
                for (var i = 0; i < attributes.length; i++) {
                queries.push(apply(readAttribute,
                    ngsiDevice,
                    lwm2mDevice,
                    attributes[i]
                ));
