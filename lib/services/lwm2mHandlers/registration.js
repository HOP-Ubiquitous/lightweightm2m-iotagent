...
function addUnsupportedAttributes(payload, configuration, deviceInformation, callback) {
    ...
    function alreadyExistent(value) {
        for ( var reg in deviceInformation.lazy) {
            if (deviceInformation.lazy[reg].name === value.name && deviceInformation.lazy[reg].type === value.type){
                return false;
            }
        }
        return true;
    ...
    if (!deviceInformation.lazy) {
        ...
        // Remove already existent in deviceInformation.lazy
        newAttributes = newAttributes.filter(alreadyExistent);
    }
